...
export const PARTNERS_REJECTED: { merchant: string; why: string }[] = [
  ...
  { merchant: 'Plushible', why: 'Delisted 2026-09-01 by Jacob. A brand call, not a technical one: the feed worked, it paid 20%, and it was the largest single vendor on the site at 331 mapped products. It simply does not fit the aesthetic. Worth knowing what went with it, because the number is large and somebody will be tempted to undo this on the count alone: 331 products, the deepest plush stock the catalogue had, and the best-shaped stock behind the plushies guide. Several term rules elsewhere were written FOR this vendor and are now unused rather than wrong, and are marked as such where they sit. Do not re-add on catalogue size; that is the only argument for it and it was heard.' },
